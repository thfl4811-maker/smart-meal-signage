...
const p=new URLSearchParams(location.search);let school=JSON.parse(localStorage.getItem('meal_school')||'null'),month=p.get('month')||new Date().toISOString().slice(0,7),data={},date='',tab='month',allergies=new Set(JSON.parse(localStorage.getItem('allergies')||'[]')),allergyOn=localStorage.getItem('allergyOn')==='1',source='api';
const signage=p.get('mode')==='signage';if(p.get('office'))school={officeCode:p.get('office'),schoolCode:p.get('school'),schoolName:p.get('name')||'학교 급식'};
window.addEventListener('DOMContentLoaded',()=>signage?bootSignage():renderHome());
function renderHome(){app.innerHTML=`<div class="shell"><header><div class="logo">🥗</div><div><h1>스마트 월간식단 뷰어</h1><p>API 또는 엑셀로 불러오고 수정본을 사이니지에 표시합니다.</p></div>${school?`<button id="change" class="ghost">학교 변경</button>`:''}</header>${school?workspace():landing()}<div id="modal" class="modal hide"></div></div>`;bind()}
function landing(){return `<section class="landing"><h2>식단 불러오기</h2><div class="source"><button id="apiMode"><b>나이스 API</b><span>학교 검색으로 자동 불러오기</span></button><label><b>월간식단 엑셀 업로드</b><span>학교 파일을 직접 분석</span><input id="excel" type="file" accept=".xlsx,.xls"></label></div><div class="search"><input id="q" placeholder="학교명 입력"><button id="find">학교 검색</button></div><div id="results"></div></section>`}
function workspace(){return `<section class="toolbar"><div><small>${source==='api'?'나이스 API':'엑셀 업로드'}</small><h2>${esc(school.schoolName)}</h2></div><input id="month" type="month" value="${month}"><button id="reload" class="primary">식단 불러오기</button><button id="filter" class="danger">🚨 알레르기</button><button id="signage" class="dark">사이니지</button></section><nav>${[['month','월간 식단'],['day','오늘의 식단'],['edit','월 전체 편집'],['detail','상세 편집']].map(x=>`<button data-tab="${x[0]}" class="${tab===x[0]?'active':''}">${x[1]}</button>`).join('')}</nav><main id="content"><div class="loading">식단을 불러오는 중입니다.</div></main>`}
function bind(){if($('#change'))$('#change').onclick=()=>{localStorage.removeItem('meal_school');school=null;data={};renderHome()};if($('#find')){$('#find').onclick=findSchools;$('#q').onkeydown=e=>e.key==='Enter'&&findSchools()}if($('#excel'))$('#excel').onchange=loadExcel;if($('#reload'))$('#reload').onclick=loadApi;if($('#month'))$('#month').onchange=e=>{month=e.target.value;loadApi()};if($('#filter'))$('#filter').onclick=openFilter;if($('#signage'))$('#signage').onclick=openSignage;$$('[data-tab]').forEach(b=>b.onclick=()=>{tab=b.dataset.tab;$$('[data-tab]').forEach(x=>x.classList.toggle('active',x===b));renderTab()});if(school&&source==='api')loadApi()}
async function findSchools(){const q=$('#q').value.trim();if(q.length<2)return alert('두 글자 이상 입력하세요.');$('#results').innerHTML='검색 중…';const r=await fetch('/api/schools?q='+encodeURIComponent(q)),j=await r.json();$('#results').innerHTML=j.map((s,i)=>`<button class="school" data-i="${i}"><b>${esc(s.schoolName)}</b><span>${esc(s.officeName)} · ${esc(s.address)}</span></button>`).join('');$$('.school').forEach(b=>b.onclick=()=>{school=j[+b.dataset.i];source='api';localStorage.setItem('meal_school',JSON.stringify(school));renderHome()})}
async function loadApi(){try{$('#content').innerHTML='<div class="loading">나이스 식단을 불러오는 중입니다.</div>';const r=await fetch(`/api/meals?office=${school.officeCode}&school=${school.schoolCode}&month=${month.replace('-','')}`),rows=await r.json();if(!r.ok)throw Error(rows.error);data=parseRows(rows);await applySaved();setDefaultDate();renderTab()}catch(e){$('#content').innerHTML=`<div class="empty">${esc(e.message)}</div>`}}
...
function education(x){const names=x.items.map(i=>i.name).join(' '),arr=[];if(/국|찌개|탕|면/.test(names))arr.push(['🧂 나트륨','국물은 적게, 건더기 위주로 먹어요.']);if(/주스|아이스크림|케이크|빵|초콜릿/.test(names))arr.push(['🍭 당류','달콤한 후식은 정량만 먹고 물을 충분히 마셔요.']);if(/샐러드|나물|채소|쌈/.test(names))arr.push(['🥬 식이섬유','채소 반찬을 함께 먹어 장 건강을 지켜요.']);return (arr.length?arr:[['🥗 균형식','다양한 음식을 골고루 먹어요.']]).map(a=>`<div class="eduitem"><b>${a[0]}</b><p>${a[1]}</p></div>`).join('')}
function filterBanner(){if(!allergyOn||!allergies.size)return'';return `<div class="filterbar"><b>🚨 알레르기 필터 적용 중</b>${[...allergies].map(n=>`<span>${n} ${A[n]}</span>`).join('')}</div>`}
function move(n){const ds=Object.keys(data).sort(),i=ds.indexOf(date);if(ds[i+n]){date=ds[i+n];renderDay()}}
function renderEdit(){const rows=Object.keys(data).sort().flatMap(d=>data[d].items.map((i,k)=>`<tr data-d="${d}" data-i="${k}"><td>${d.slice(5)}</td><td><input class="name" value="${esc(i.name)}"></td><td><input class="alg" value="${i.allergy.join(',')}"></td><td><select class="type"><option value="normal">일반</option><option value="choiceA" ${i.type==='choiceA'?'selected':''}>선택 A</option><option value="choiceB" ${i.type==='choiceB'?'selected':''}>선택 B</option><option value="self" ${i.type==='self'?'selected':''}>자율배식</option></select></td><td><button class="ghost saveRow">적용</button></td></tr>`)).join('');$('#content').innerHTML=`<section class="editor"><div class="edithead"><h2>월 전체 편집</h2><div><button id="replace" class="ghost">찾아 바꾸기</button><button id="saveAll" class="primary">전체 저장</button></div></div><div class="table"><table><thead><tr><th>날짜</th><th>메뉴명</th><th>알레르기</th><th>표시</th><th></th></tr></thead><tbody>${rows}</tbody></table></div></section>`;$$('.saveRow').forEach(b=>b.onclick=()=>applyRow(b.closest('tr')));$('#replace').onclick=replaceAll;$('#saveAll').onclick=saveProject}
